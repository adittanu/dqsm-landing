    ...
    <>
      <section className="bg-hero-gradient py-20">
        <div className="container-x">
          <SectionHeading
            eyebrow="Blog"
            title="Insights, tutorials, and use cases"
            description="Learn from the DQSM team and community how to build quizzes that convert, engage, and educate."
          />
          <div className="mt-8 flex flex-wrap items-center justify-center gap-2">
            {blogCategories.map((c) => (
              <button
                key={c}
                className="rounded-full px-4 py-1.5 text-sm font-medium bg-white text-ink-700 border border-ink-100 hover:border-brand-300 transition"
              >
                {c}
              </button>
            ))}
            ...
            >
              ...
              <div className="p-5">
                ...
                <div className="mt-4 flex items-center justify-between text-xs text-ink-400">
                  ...
                  <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" /> {p.date} · {p.readTime}</span>
                </div>
              </div>
            </Link>
          ))}
